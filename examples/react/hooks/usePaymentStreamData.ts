...
export function usePaymentStreamData(chainName: string) {
  ...
  const [receiverPagination, setReceiverPagination] = useState(
    paginate(50n, true)
  );
  ...
  const paymentStreamsQueryAsSender =
    mainchain.stream.v1.useAllStreamsForSender({
      request: {
        senderAddr: address || "",
  ...
  const paymentStreamsQueryAsReceiver =
    mainchain.stream.v1.useAllStreamsForReceiver({
      request: {
  ...
  const loading = isFetching || isStaticQueriesFetching;
