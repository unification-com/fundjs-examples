...
export const WebSocket = ({ chainName }: WebSocketProps) => {
  ...
  function parseStreamDeposit(
    attributes: any[],
    txHash: string
  ): React.JSX.Element {
    const sender = formatExplorerUrl(
      getAttr(attributes, "sender"),
      "account",
      true
    );
    const receiver = formatExplorerUrl(
      getAttr(attributes, "receiver"),
      "account",
      true
    );
    ...
    return (
      <>
        Tx {txHashLink} - Top Up deposit: {sender} -&gt; {receiver}:{" "}
        {amountFund} {chainCoin.symbol}
      </>
    );
  }

  function parseCreateStream(
    attributes: any[],
    txHash: string
  ): React.JSX.Element {
    const sender = formatExplorerUrl(
    ...
    return (
      <>
        Tx {txHashLink} - Create Stream: {sender} -&gt; {receiver}:{" "}
        {flowRateFund} {chainCoin.symbol}/sec
      </>
    );
  }

  function parseClaimStream(
    attributes: any[],
    txHash: string
  ): React.JSX.Element {
    const sender = formatExplorerUrl(
    ...
    const amountValFeeFund = exponentiate(
      amountValFee[0].amount,
      -exponent
    ).toFixed(3);
    const txHashLink = formatExplorerUrl(txHash, "tx", true);
    return (
      <>
        Tx {txHashLink} - Claim Stream: {sender} -&gt; {receiver}: Received{" "}
  ...
  function parseUpdateFlowRate(
    ...
    return (
      <>
        Tx {txHashLink} - Update Flow Rate: {sender} -&gt; {receiver} from{" "}
        {oldFlowRateFund} to {newFlowRateFund} {chainCoin.symbol}/sec
      </>
    );
  }

  function parseCancelStream(
    attributes: any[],
    txHash: string
  ): React.JSX.Element {
    const sender = formatExplorerUrl(
    ...
    const amountRefundedFund = exponentiate(
      amountRefunded[0].amount,
      -exponent
    ).toFixed(3);

    return (
      <>
        Tx {txHashLink} - Stream Cancelled: {sender} -&gt; {receiver}. Refunded{" "}
