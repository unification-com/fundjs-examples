...
createMainchainRPCQueryClient({rpcEndpoint: RPC_ENDPOINT})
    .then(async mainchainClient => {
        ...
        // However, it does not contain IBC endpoints, so we also need to create a client for IBC queries
    ...
    const flowRate = await clients.mainchainClient.mainchain.stream.v1.calculateFlowRate({
    ...
    // got some IBC info, using the IBC client
